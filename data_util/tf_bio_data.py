...
class Featurizer():
    ...
    def __init__(self, atom_codes=None, atom_labels=None,
        ...
        if atom_codes is not None:
            ...
            if atom_labels is not None:
                ...
                atom_labels = ['atom%s' %
                               i for i in range(self.NUM_ATOM_CLASSES)]
    ...
    def get_features(self, molecule, molcode=None):
        ...
        heavy_atoms = []
        n = self.NAMED_PROPS
        n = list(map(lambda x: x.replace('heavyvalence', 'heavydegree'), n))
        n = list(map(lambda x: x.replace('heterovalence', 'heterodegree'), n))
        self.NAMED_PROPS = n
