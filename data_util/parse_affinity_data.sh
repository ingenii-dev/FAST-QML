...
cat $path/refined-set/index/INDEX_general_PL_data.2016 | while read l1 l2 l3 l4 l5; do
    if [[ ! $l1 =~ "#" ]]; then
        echo $l1,$l4
    fi
...
grep -v '#' $path/refined-set/index/INDEX_refined_data.2016 | cut -f 1 -d ' ' > refined.csv

grep -v '#' $path/refined-set/index/INDEX_core_data.2016 | cut -f 1 -d ' '  > core.csv
