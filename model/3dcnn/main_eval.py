...
parser.add_argument(
    "--data-dir", default="../../data/processed", help="dataset directory")
parser.add_argument("--dataset-type", type=float,
                    default=1, help="1: ml-hdf, 2: ml-hdf v2")
parser.add_argument(
    "--csv-fn", default="core_test_3dnn.csv", help="csv file path")
parser.add_argument(
    "--mlhdf-fn", default="pdbbind2016_core_test.hdf", help="ml-hdf path")
parser.add_argument("--model-path", default="../../data/pdbbind2016_refined_model.pth",
                    help="model checkpoint file path")
parser.add_argument("--complex-type", type=int, default=1,
                    help="1: crystal, 2: docking")
parser.add_argument("--rmsd-threshold", type=float, default=20,
...
def eval():
    ...
    dataset = Dataset_MLHDF(os.path.join(args.data_dir, args.mlhdf_fn), args.dataset_type, os.path.join(
        args.data_dir, args.csv_fn), is_crystal=is_crystal, rmsd_weight=False, rmsd_thres=args.rmsd_threshold)
    ...
    print("RMSE: %.3f, MAE: %.3f, R^2 score: %.3f, Pearson: %.3f, Spearman: %.3f, mean/std: %.3f/%.3f" %
          (rmse, mae, r2, pearson, spearman, mean, std))

    if args.save_pred:
        csv_fpath = "%s_%s_pred.csv" % (
            args.model_path[:-4], args.mlhdf_fn[:-4])
        df = pd.DataFrame(pred_list, columns=["cid", "label", "pred"])
        df.to_csv(csv_fpath, index=False)

    if args.save_feat:
        npy_fpath = "%s_%s_feat.npy" % (
            args.model_path[:-4], args.mlhdf_fn[:-4])
